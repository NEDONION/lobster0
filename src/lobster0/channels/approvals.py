...
from lobster0.storage.tooling import ApprovalPresentation

_APPROVE = re.compile(r"/approve[ \t]+([1-9][0-9]*)[ \t]+(once|session|always)\Z")
_DENY = re.compile(r"/deny[ \t]+([1-9][0-9]*)\Z")
_ACTION_V1_KEYS = frozenset({"lobster0_action", "approval_id", "decision"})
_ACTION_V2_KEYS = frozenset({"version", "lobster0_action", "approval_id", "decision"})
...
class ChannelApprovalController:
    ...
    async def handle_card_action(
        ...
        *,
        ...
        """只接受 Lobster0 自己生成且键集合完全一致的按钮 payload。"""
...
def feishu_approval_status_card(
    ...
    presentations = {
        "processing": ("orange", "Lobster0 审批 · 处理中", "正在验证并执行这次操作。"),
        "succeeded": ("green", "Lobster0 审批 · 已完成", "操作已完成。"),
        "denied": ("red", "Lobster0 审批 · 已拒绝", "操作已拒绝，未执行。"),
        "failed": ("red", "Lobster0 审批 · 处理失败", "操作未完成。"),
    }
    try:
...
def text_approval_prompt(envelope: ApprovalEnvelope) -> str:
    ...
    return (
        f"Lobster0 审批 #{envelope.approval_id}\n"
...
def parse_approval_card_action(value: Any) -> tuple[int, ApprovalDecision] | None:
    ...
    if "version" in value and (type(value["version"]) is not int or value["version"] != 2):
        return None
    if value.get("lobster0_action") != "approval":
        return None
    approval_id = value.get("approval_id")
...
def _approval_card(
    ...
    return {
        ...
        "header": {
            ...
            "title": {
                "tag": "plain_text",
                "content": f"Lobster0 审批 #{envelope.approval_id}",
            },
        },
        "elements": [
            {
