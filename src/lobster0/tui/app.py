...
from lobster0.agent.events import RunEvent, display_tool_arguments
...
from lobster0.storage.database import DatabaseError
from lobster0.storage.migrations import MigrationError
...
_ERROR_SUMMARIES = {
    ...
    "en": {
        ...
        "turn_deadline": "the turn exceeded its wall-clock budget",
...
_TEXT = {
    ...
    "en": {
        ...
        "initialize": "Initialize Lobster0",
...
def run_tui(paths: StatePaths) -> int:
    """运行 Lobster0 全屏应用并返回稳定进程退出码。"""
    runtime = _load_runtime(paths) if _is_initialized(paths) else None
    result = Lobster0App(paths, runtime=runtime).run()
