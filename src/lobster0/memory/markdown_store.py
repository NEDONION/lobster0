...
class MarkdownUnitDocument:
    ...
    def __post_init__(self) -> None:
        ...
        if "<!-- lobster0:" in self.text or "```lobster0-memory" in self.text:
...
class MemoryMarkdownStore:
    ...
    def append(self, unit: MarkdownUnitDocument) -> MarkdownWrite:
        ...
        try:
            ...
            marker = f"<!-- lobster0:unit {unit.unit_id} -->".encode()
    ...
    def upsert(self, unit: MarkdownUnitDocument) -> MarkdownWrite:
        ...
        try:
            ...
            block = _unit_block(unit)
            block_hash = hashlib.sha256(block).hexdigest()
            start_marker = f"<!-- lobster0:unit {unit.unit_id} -->".encode()
            end_marker = f"<!-- lobster0:end {unit.unit_id} -->".encode()
...
def _unit_block(unit: MarkdownUnitDocument) -> bytes:
    ...
    return (
        f"<!-- lobster0:unit {unit.unit_id} -->\n"
        f"## {unit.key}\n\n"
        f"{unit.text.strip()}\n\n"
        "```lobster0-memory\n"
        f"{encoded}\n"
        "```\n"
        f"<!-- lobster0:end {unit.unit_id} -->\n"
    ).encode()


def _replace_or_append(payload: bytes, unit_id: str, block: bytes) -> bytes:
    """在内存中的完整 Markdown 文档替换一个 block，缺失时追加。"""
    start_marker = f"<!-- lobster0:unit {unit_id} -->".encode()
    end_marker = f"<!-- lobster0:end {unit_id} -->".encode()
