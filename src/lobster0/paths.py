...
def resolve_home(
    ...
    Args:
        ...
        environ: 用于读取 ``LOBSTER0_HOME`` 的环境；默认使用当前进程环境。
    ...
    selected: str | Path = (
        ...
        else source.get("LOBSTER0_HOME") or Path.home() / ".lobster0"
    )
    candidate = Path(selected).expanduser()
    if not candidate.is_absolute():
        raise PathConfigurationError("Lobster0 home must be an absolute path")
...
def build_state_paths(home: Path) -> StatePaths:
    ...
    return StatePaths(
        ...
        database=resolved_home / "lobster0.db",
