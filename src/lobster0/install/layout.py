...
from lobster0.install.models import InstallError, InstallPlan, InstallRequest
from lobster0.install.platforms import _production_identity, _resolve_invoking_user
...
_SYSTEM_PREFIX = Path("/usr/local/lib/lobster0")
_SYSTEM_COMMAND = Path("/usr/local/bin/lobster0")
...
class InstallLayout:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.launcher != self.bin_dir / "lobster0"
        ...
        if self.program_prefix == _SYSTEM_PREFIX:
            if self.command_link != _SYSTEM_COMMAND:
                _request_invalid()
        elif self.command_link.parts[-3:] != (".local", "bin", "lobster0"):
            _request_invalid()
    ...
    def user(cls, home: Path, *, version: str) -> InstallLayout:
        ...
        Returns:
            `~/.lobster0` 程序/状态根和 `~/.local/bin/lobster0` 命令路径。
        ...
        if not _safe_absolute_path(home) or home == Path("/"):
            _request_invalid()
        uid = os.geteuid()
        prefix = home / ".lobster0"
        return cls._build(
            prefix,
            prefix,
            home / ".local" / "bin" / "lobster0",
            version,
            owner_uid=uid,
            user_home=home,
        )
    ...
    def for_request(
        ...
        prefix = request.prefix if request.prefix is not None else home / ".lobster0"
        return cls._build(
            prefix,
            state_home,
            home / ".local" / "bin" / "lobster0",
    ...
    def for_plan(cls, plan: InstallPlan) -> InstallLayout:
        ...
        if system:
            ...
            command = home / ".local" / "bin" / "lobster0"
        return cls._build(
            plan.program_prefix,
            plan.state_home,
            command,
    ...
    def _build(
        ...
        *,
        ...
        if user_home is None:
            ...
            elif command_link.parts[-3:] == (".local", "bin", "lobster0"):
        ...
        if system:
            ...
            if command_link != user_home / ".local" / "bin" / "lobster0":
                _request_invalid()
