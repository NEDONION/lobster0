...
from lobster0.install.artifacts import (
    ExtractionLimits,
    _Opener,
    _read_bounded_url,
    download_artifact,
...
from lobster0.install.releases import resolve_release_source
...
class _SystemOperations:
    ...
    def commit(
        ...
        receipt = InstallReceipt(
            ...
            managed_files=(("bin/lobster0", launcher_hash),),
...
def _environment_assignments(environment: dict[str, str]) -> tuple[str, ...]:
    ...
    Args:
        environment: `_target_environment` 及可选 Lobster0 path 字段。
...
def _validate_and_import_config(
    ...
    code = (
        "from pathlib import Path;from lobster0.paths import build_state_paths;"
        "from lobster0.config import load_config;import sys;"
...
def _service_inputs_complete(config: Path, secrets: Path) -> bool:
    ...
    if type(provider) is not dict:
        return False
    api_key_env = provider.get("api_key_env", "LOBSTER0_MODEL_API_KEY")
...
def _service_inputs_complete_as_owner(
    ...
    code = (
        ...
        "from lobster0.install.orchestrator import _service_inputs_complete;"
...
def _import_config_as_owner(
    ...
    code = (
        ...
        "from lobster0.config import load_config;from lobster0.paths import build_state_paths;"
        ...
        "tmp=tempfile.TemporaryDirectory(prefix='lobster0-config-');p=Path(tmp.name);"
...
def _import_secrets_as_owner(
    ...
    code = (
        ...
        "from lobster0.install.orchestrator import _parse_secret_names;"
...
def _install_service_as_owner(
    ...
    environment = _target_environment(account)
    code = (
        "import json;from pathlib import Path;from lobster0.install.layout import InstallLayout;"
        "from lobster0.install.service import ServicePlatform,render_service_spec,service_install;"
...
def _uninstall_service_as_owner(
    ...
    environment = _target_environment(account)
    code = (
        "import sys;from pathlib import Path;from lobster0.install.layout import InstallLayout;"
        "from lobster0.install.service import ServicePlatform,render_service_spec,"
        "service_uninstall;"
        "l=InstallLayout._build(Path(sys.argv[1]),Path(sys.argv[2]),Path(sys.argv[3]),"
        "sys.argv[4],owner_uid=int(sys.argv[5]),user_home=Path(sys.argv[6]));"
        "s=render_service_spec(l,ServicePlatform(sys.argv[7]));"
