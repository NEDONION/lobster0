...
_LOBSTER0_REPOSITORY = "https://github.com/NEDONION/lobster0"
_NODE_REPOSITORY = "https://github.com/nodejs/node"
_SOURCE_REPOSITORIES = {_LOBSTER0_REPOSITORY, _NODE_REPOSITORY}
_ARTIFACT_RULES = {
    "wheel": (frozenset({"application/zip"}), "universal", _LOBSTER0_REPOSITORY, False),
    "sdist": (frozenset({"application/gzip"}), "universal", _LOBSTER0_REPOSITORY, False),
    "requirements": (frozenset({"text/plain"}), "universal", _LOBSTER0_REPOSITORY, False),
    "node": (frozenset({"application/gzip"}), "concrete", _NODE_REPOSITORY, True),
    "tui": (frozenset({"application/gzip"}), "concrete", _LOBSTER0_REPOSITORY, False),
    "sandbox-image": (frozenset({"text/plain"}), "universal", _LOBSTER0_REPOSITORY, False),
    "runtime-image": (frozenset({"text/plain"}), "universal", _LOBSTER0_REPOSITORY, False),
    "installer": (frozenset({"application/zip"}), "universal", _LOBSTER0_REPOSITORY, False),
    "sbom": (
        frozenset({"application/vnd.cyclonedx+json", "application/spdx+json"}),
        "universal",
        _LOBSTER0_REPOSITORY,
        False,
    ),
}
_ERROR_CODES = {
...
class ReleaseManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.product != "lobster0" or self.python != "3.12":
            _invalid("product")
    ...
    def _validate_release_urls(self) -> None:
        """把 Lobster0 GitHub asset tag 绑定到 manifest 自身版本。"""
        expected = f"/NEDONION/lobster0/releases/download/v{self.version}/"
        for artifact in self.artifacts:
...
def _artifact_filename_matches(artifact: Artifact) -> bool:
    ...
    expected = {
        "wheel": f"lobster0_agent-{python_version}-py3-none-any.whl",
        "sdist": f"lobster0_agent-{python_version}.tar.gz",
        "requirements": "requirements-all.lock",
        "node": f"lobster0-node-{version}-{platform}.tar.gz",
        "tui": f"lobster0-tui-{version}-{platform}.tar.gz",
        "installer": "lobster0-installer.pyz",
    }
...
def _validate_artifact_url(value: object) -> None:
    ...
    if (
        ...
        or len(segments) != 7
        or segments[1:5] != ["NEDONION", "lobster0", "releases", "download"]
