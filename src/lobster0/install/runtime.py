...
from lobster0.install.layout import InstallLayout, InstallLock, _program_mode
...
from lobster0.install.receipt import InstallReceipt, _rename_no_replace
...
class RuntimeReceipt:
    ...
    Args:
        version: Lobster0 Release SemVer。
...
class RuntimeBuilder:
    ...
    def build(self, inputs: RuntimeInputs) -> RuntimeReceipt:
        ...
        try:
            ...
            _copy_verified_file(
                tokens["installer"],
                inputs.layout.staging / "lobster0-installer.pyz",
                0o700,
            )
    ...
    def _smoke(
        ...
        *,
        ...
        version = self._checked(
            (str(python), "-I", "-m", "lobster0", "--version"),
            env,
            _SMOKE_TIMEOUT_SECONDS,
            (python_token, config_token),
            (python_link,),
        )
        if version.stdout != f"lobster0 {inputs.manifest.version}\n".encode():
            _runtime_failed()
        install = self._checked(
            (str(python), "-I", "-m", "lobster0", "install-smoke", "--json"),
            env,
            _SMOKE_TIMEOUT_SECONDS,
            (python_token, config_token),
            (python_link,),
...
def _inspect_wheel(path: Path, version: str) -> None:
    """从 no-follow wheel descriptor 校验 Name/Version 和 lobster0 console entry。"""
    descriptor = -1
    parent = -1
    try:
    ...
    try:
        ...
        with stream, zipfile.ZipFile(stream) as archive:
            ...
            expected_dist_info = f"lobster0_agent-{python_version}.dist-info"
            metadata_names = [
                info.filename
                for info in infos
            ...
            if (
                metadata.get("Name") != "lobster0-agent"
                or metadata.get("Version") != python_version
                or not parser.has_section("console_scripts")
                or parser.get("console_scripts", "lobster0", fallback="")
                != "lobster0.cli:main"
            ):
                _runtime_failed()
        after = os.stat(name, dir_fd=parent, follow_symlinks=False)
