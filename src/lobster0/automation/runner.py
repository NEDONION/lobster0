...
from lobster0.automation.models import RunStatus, TaskResponse, TaskRun, TaskRunSnapshot
...
class TaskRunner:
    ...
    async def run_once(self, worker_id: str, now: datetime) -> TaskRunAttempt | None:
        ...
        renewal = asyncio.create_task(
            ...
            name=f"lobster0-task-{claimed.id}-lease-renew",
        )
        try:
            profile = TurnExecutionProfile(
                source="automation",
    ...
    async def start(self) -> None:
        ...
        self._workers = tuple(
            asyncio.create_task(
                ...
                name=f"lobster0-automation-worker-{index + 1}",
            )
