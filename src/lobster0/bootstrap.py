"""Lobster0 状态目录、模板、数据库和 Owner 的幂等初始化。"""
...
from lobster0.storage.repositories import Owner, OwnerRepository
...
def initialize_state(
    ...
    *,
    ...
    """幂等创建一个可加载、可迁移的单 Owner Lobster0 状态目录。
...
def render_default_config(
    ...
    *,
    ...
    Args:
        ...
        sandbox_image: 安装器提供的 Lobster0 GHCR sha256 digest；省略则保留开发默认值。
    ...
    """
    image = "lobster0-sandbox:phase6" if sandbox_image is None else sandbox_image
    if sandbox_image is not None and re.fullmatch(
        r"ghcr\.io/nedonion/lobster0-sandbox@sha256:[0-9a-f]{64}", image
    ) is None:
        raise ConfigError("sandbox image must be a digest-pinned Lobster0 GHCR image")
