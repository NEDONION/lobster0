"""把 Lobster0 身份文件和会话历史构造成模型请求。"""

import hashlib
import json
from pathlib import Path

from lobster0.browser.models import preserve_browser_provenance
from lobster0.memory.context import MemoryContextSelector
from lobster0.memory.models import DisclosureContext, DisclosureDecision
...
from lobster0.memory.store import MemoryError, MemorySnapshot, MemoryStore
from lobster0.paths import StatePaths
from lobster0.providers.base import JsonValue, ModelMessage, ModelRequest
from lobster0.skills.loader import SkillError, SkillLoader

_SYSTEM_PREAMBLE_EN = (
    "You are Lobster0, a private self-hosted personal agent. "
    ...
    "use memory_forget or memory_correct only for the owner's explicit request. "
    "Memory review decisions belong to the owner UI; never approve or reject a review. "
...
_SYSTEM_PREAMBLE_ZH = (
    ...
    "只有 Owner 明确要求遗忘或纠错时，才使用 memory_forget 或 memory_correct；"
    "Memory Review 只能由 Owner 在本地 UI 决定，绝不能代表 Owner 批准或拒绝。"
...
class ContextBuilder:
    ...
    def build(
        ...
        *,
        ...
        try:
            ...
            raise ContextError("cannot authorize Lobster0 memory disclosure") from error
        except MemoryError as error:
            raise ContextError("cannot read Lobster0 memory files") from error
        query = next(
        ...
        try:
            ...
            raise ContextError("cannot load Lobster0 skills") from error
        skill_text = "\n\n".join(
    ...
    def _read_identity(path: Path) -> str:
        ...
        try:
            ...
            raise ContextError(f"cannot read Lobster0 identity file {path}") from error
