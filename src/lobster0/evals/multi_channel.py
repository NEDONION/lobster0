...
from lobster0.channels.discord_rendering import render_discord_text
from lobster0.channels.manager import ChannelManager
from lobster0.channels.supervisor import ChannelRuntime, GatewaySupervisor
from lobster0.channels.telegram import TelegramAdapter
from lobster0.config import DiscordConfig, TelegramConfig
...
def _dedupe_evidence(platform: str) -> tuple[str, ...]:
    ...
    with TemporaryDirectory(prefix="lobster0-multi-dedupe-") as directory:
...
async def _read_tool_evidence(case: EvalCase) -> tuple[str, ...]:
    ...
    with TemporaryDirectory(prefix="lobster0-multi-tool-") as directory:
        paths = build_state_paths(Path(directory) / "state")
...
async def _delivery_evidence(platform: str) -> tuple[str, ...]:
    ...
    with TemporaryDirectory(prefix="lobster0-multi-delivery-") as directory:
        paths = build_state_paths(Path(directory) / "state")
        owner = initialize_state(paths).owner
...
async def _restart_evidence(platform: str) -> tuple[str, ...]:
    ...
    with TemporaryDirectory(prefix="lobster0-multi-restart-") as directory:
        paths = build_state_paths(Path(directory) / "state")
        owner = initialize_state(paths).owner
        database = Database(paths.database)
...
async def _isolation_evidence(platform: str) -> tuple[str, ...]:
    ...
    try:
        ...
        with TemporaryDirectory(prefix="lobster0-multi-isolation-") as directory:
            paths = build_state_paths(Path(directory) / "state")
            owner = initialize_state(paths).owner
            database = Database(paths.database)
            deliveries = DeliveryRepository(database)
