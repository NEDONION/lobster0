...
from lobster0.config import AppConfig, load_config
from lobster0.env import load_dotenv, resolve_dotenv_path
from lobster0.evals.feishu_automation_live import validate_automation_evidence_report
from lobster0.evals.feishu_live import validate_evidence_report
...
from lobster0.gateway import validate_gateway_environment
from lobster0.gateway_service import (
    LaunchdService,
    ServiceSpec,
    ServiceStatus,
)
from lobster0.paths import StatePaths, build_state_paths, resolve_home
...
def _load_installed_service(
    ...
    *,
    ...
    plist_path = launch_agents / "io.lobster0.gateway.plist"
    ...
    if (
        payload.get("Label") != "io.lobster0.gateway"
        or not isinstance(arguments, list)
        or len(arguments) != 4
        or arguments[1:3] != ["gateway", "--home"]
        or Path(arguments[3]) != paths.home
        or not isinstance(environment, dict)
        or environment.get("LOBSTER0_GATEWAY_COMMIT") != expected_commit
...
def _send_recovery_probe(runtime: ProductionRuntime) -> str:
    ...
    message = MessageRepository(Database(runtime.paths.database)).create_channel_notice(
        ...
        f"Lobster0 Phase 6 recovery probe · {marker}",
    )
