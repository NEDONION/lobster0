"""Lobster0 SQLite Schema 的显式单向迁移。"""
...
_MIGRATION_RESOURCES = {
    ...
    7: "migrations/0007_controlled_evolution.sql",
    8: "migrations/0008_evolution_approvals.sql",
    9: "migrations/0009_user_upload_artifacts.sql",
    10: "migrations/0010_inbound_replied_to.sql",
}
...
def apply_migrations(database: Database) -> tuple[int, ...]:
    ...
    try:
        with database.connect() as connection:
            ...
            if newest_version > LATEST_SCHEMA_VERSION:
                raise MigrationError(
                    ...
                    f"this Lobster0 supports {LATEST_SCHEMA_VERSION}"
                )
        ...
        raise MigrationError(
            f"failed to apply Lobster0 schema migration {applying_version}"
        ) from error
...
def _load_migration(version: int) -> str:
    ...
    try:
        ...
        raise MigrationError(f"missing Lobster0 schema migration {version}") from error
    resource = resources.files("lobster0.storage")
