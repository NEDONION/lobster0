"""管理 Lobster0 自有的用户级 macOS LaunchAgent。"""
...
def render_launchd_service(
    *,
    ...
    """渲染固定 label/argv 的 Lobster0 LaunchAgent。

    Args:
        launcher: managed runtime 中的 ``lobster0`` console launcher。
    ...
    payload = {
        "EnvironmentVariables": {
            "LOBSTER0_ENV_FILE": str(dotenv_path),
            "LOBSTER0_GATEWAY_COMMIT": commit,
        },
...
class LaunchdService:
    """只安装、查询、重启和删除 Lobster0 自己拥有的 LaunchAgent。"""
    ...
    def status(self) -> ServiceStatus:
        ...
        Raises:
            ServiceError: 平台不支持或已有同名文件不属于 Lobster0。
        """
