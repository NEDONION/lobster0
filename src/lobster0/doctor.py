...
from lobster0.policy.network import NetworkPolicyError, normalize_network_rule
from lobster0.sandbox.base import SandboxUnavailableError
from lobster0.sandbox.docker import discover_rootless_client_transport
...
from lobster0.tui_launcher import inspect_pi_tui, is_supported_node_version
...
def _check_browser(paths: StatePaths, config: AppConfig | None) -> CheckResult:
    ...
    lock = profile / ".lobster0-browser.lock"
...
def _check_database(paths: StatePaths) -> CheckResult:
    ...
    if not paths.database.is_file() or paths.database.is_symlink():
        return CheckResult("database", CheckStatus.FAIL, "lobster0.db is missing or unsafe")
