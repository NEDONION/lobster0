...
def _escape_raw_html(line: str) -> str:
    """转义代码外 HTML，并把普通文本中的裸 URL 转为可点击链接。"""
    def escape_tag(match: re.Match[str]) -> str:
        """转义单个已识别的 HTML 标签。"""
        return match.group(0).replace("<", "&lt;").replace(">", "&gt;")

    return "".join(
        segment if is_code else _linkify_bare_urls(_HTML_TAG.sub(escape_tag, segment))
        for segment, is_code in _inline_code_segments(line)
    )


def _linkify_bare_urls(text: str) -> str:
    """把非代码文本中的 HTTP(S) 裸 URL 转为 Markdown 链接。"""
    def replace_url(match: re.Match[str]) -> str:
        """保留 URL 后的句末标点，并返回显式 Markdown 链接。"""
        matched = match.group(0)
        target = matched.rstrip(_URL_TRAILING_PUNCTUATION)
        return f"[{target}]({target}){matched[len(target):]}"

    return _BARE_URL.sub(replace_url, text)
