...
from miniclaw.channels.feishu import FeishuAdapter, FeishuTransport
...
def _dedupe_evidence() -> tuple[str, ...]:
    ...
    with TemporaryDirectory(prefix="miniclaw-channel-eval-") as directory:
...
async def _read_tool_evidence(case: EvalCase) -> tuple[str, ...]:
    ...
    with TemporaryDirectory(prefix="miniclaw-channel-tool-") as directory:
        paths = build_state_paths(Path(directory) / "state")
        ...
        if not result.ok or "MINICLAW-FEISHU-GROUNDED" not in str(result.data):
...
async def _restart_queued_evidence() -> tuple[str, ...]:
    ...
    with TemporaryDirectory(prefix="miniclaw-channel-restart-") as directory:
        paths = build_state_paths(Path(directory) / "state")
...
async def _restart_running_evidence() -> tuple[str, ...]:
    ...
    with TemporaryDirectory(prefix="miniclaw-channel-running-") as directory:
        paths = build_state_paths(Path(directory) / "state")
        owner = initialize_state(paths).owner
        database = Database(paths.database)
        inbound = InboundEventRepository(database)
...
async def _delivery_retry_evidence() -> tuple[str, ...]:
    ...
    with TemporaryDirectory(prefix="miniclaw-channel-delivery-") as directory:
        paths = build_state_paths(Path(directory) / "state")
        owner = initialize_state(paths).owner
        database = Database(paths.database)
        clock = _MutableClock()
...
async def _card_fallback_evidence() -> tuple[str, ...]:
    ...
    with TemporaryDirectory(prefix="miniclaw-channel-card-") as directory:
        paths = build_state_paths(Path(directory) / "state")
        owner = initialize_state(paths).owner
        database = Database(paths.database)
