...
- Produces: `InstallLayout.for_request(request, user)`, `InstallLayout.for_plan(plan)`, `InstallLock.acquire(layout)`, `InstallLock.owns(layout) -> bool`, `InstallReceipt.load/write`, `render_launcher(layout) -> bytes`, `managed_file_sha256(path)`.
...
- Produces: `RuntimeBuilder.build(...) -> RuntimeReceipt`, `RuntimeBuilder.smoke(...)`, `activate_runtime(layout, receipt)`, `discard_interrupted_runtime_staging(layout, lock, manifest)`, `discard_unactivated_runtime(layout, lock)`, `retain_current_and_previous(layout)`.
...
`RuntimeBuilder` creates a 0600, `O_EXCL`/no-follow recovery marker as the first entry in a new staging directory and fsyncs the marker plus directory before any other build output. The marker binds the canonical full manifest, relative staging identity and created directory/file inode facts. `discard_interrupted_runtime_staging(layout, lock, manifest)` requires the same actual live lock, exact manifest-bound marker, stable full-tree facts and unchanged marker token around every reference scan and quarantine transition. It protects `current`, `current.next` and install-receipt references; missing, malformed, foreign or concurrently replaced markers/trees fail closed, while pre-commit failures restore the exact inode or retain durable 0700 private evidence.
...
class Installer:
    ...
    def run(self, request: InstallRequest) -> InstallResult:
        ...
        self.emit(InstallEvent("install.preflight", "ok", None, "manifest"))
        if request.dry_run:
...
`InstallEvent.detail` remains the Task 4 fixed allowlist and never carries `InstallPlan.safe_summary()` or another dynamic summary string. After ordered events, the CLI emits one final structured result event with separately validated `action`, `version`, `platform.os`, `platform.arch` and `changed` fields; human mode renders the same fields to stderr. This exposes the dry-run target without reopening the arbitrary detail channel.
