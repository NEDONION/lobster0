...
| Gate | 当前记录 |
| --- | --- |
| Python suite size | 508 tests；文档提交前最终 fresh pass 待运行 |
| TypeScript | 27 tests；最终 fresh pass 待运行 |
| active offline Agent | 28/28；最终 fresh pass 待运行 |
| retained Feishu Channel | 12/12；实现关联回归已通过 |
| Telegram + Discord Channel | 20/20；最终 full gate 待运行 |
| combined Channel | 32/32；最终 full gate 待运行 |
| local Channel soak | 20 轮、640/640；最终 fresh pass 待运行 |
| scenario validation | 75 个 active/retained eval case |
| Feishu Live dataset | 15 条固定 ID、schema 校验通过 |
| Feishu Live harness | 离线契约与 fake subprocess/SQLite 集成通过 |
| real Feishu Bot | **REAL BOT PENDING** |
| real Feishu 15/15 | **PENDING**，没有伪造 Evidence |

最终门禁完成后，本表才会把“待运行”更新为新鲜 PASS；真实平台状态仍必须由独立 Live Evidence 决定。
...
详细操作见 [真实飞书 Bot 与 Live E2E 工程落地](../../engineering/phase-5/feishu-live-e2e.md)。
