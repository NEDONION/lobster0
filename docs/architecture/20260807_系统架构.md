...
Artifact。任意 Shell、向量数据库和自动演进尚未实现。
...
```mermaid
flowchart TD
    CMD["lobster0 argv"] --> PARSER["argparse"]
...
`src/lobster0/runtime.py` 是生产 Agent 的唯一装配点。它创建并拥有：
...
sequenceDiagram
    ...
    participant CLI as lobster0 eval
    participant Loader as JSONL validator
    participant Temp as 临时 Lobster0
    participant Turn as TurnService
...
Approval；TypeScript Worker 只解析 `lobster0.browser.v1` 的封闭 NDJSON action，并控制 Lobster0 专用 Chromium
...
sequenceDiagram
    ...
    C->>W: lobster0.browser.v1 NDJSON
...
当前架构状态是 Phase 6.5 **IMPLEMENTATION PASS**：925/925 Python、36/36 TUI TypeScript、
...
[Phase 6 Sandbox 与 Checkpoint](../engineering/phase-6/20260809_sandbox-and-checkpoint.md)；Browser 边界见
...
flowchart LR
    ...
    GP --> PG["production lobster0 gateway"]
