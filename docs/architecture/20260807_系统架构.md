...
sequenceDiagram
    ...
    CLI-->>Dev: Agent 39/39 + Channel 33/33 + Automation 15/15
```
...
- 通用 Desktop Workbench 已完成产品与架构设计，但尚无实现；未来 Electron Main 只监督 Python Bridge，Renderer
  不得直读 SQLite、Secret、Workspace 或执行 Tool。W0～W3 边界见
  [通用桌面 Agent 工作台设计](20260809_通用桌面Agent工作台设计.md)。

当前架构状态是 Phase 6 **IMPLEMENTATION PASS**：798/798 Python tests、35/35 TypeScript、39/39 Agent、
33/33 Channel、660/660 local Channel soak 与 15/15 Automation；Feishu 是
...
历史恢复、Card callback sent-receipt 绑定与 Approval continuation 已进入 `main`；Memory Autopilot A～E 与 Phase 6
Task/Scheduler/Runner/Sandbox/Checkpoint 也已通过本地 versioned gate。Feishu/Discord 严格 15/15 仍是独立 Live
Evidence 收口项；下一条功能主线是 **Phase 6.5 Browser Agent**。
...
[Phase 6 Sandbox 与 Checkpoint](../engineering/phase-6/20260809_sandbox-and-checkpoint.md)；Tool/TUI 约束见
