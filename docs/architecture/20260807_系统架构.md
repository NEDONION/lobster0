...
当前 Memory 仍是 Phase 3 的手工/审批式 v1：只自动加载 `MEMORY.md` 与今日/昨日 daily，不会从普通对话自动
提取或跨 Session 搜索。已确认但尚未实现的 Memory Autopilot 目标见
[能力 Gap 与重构架构](20260808_Memory-Autopilot能力Gap与重构架构.md)。
...
计划中的 Memory Autopilot 将把当前无条件 snapshot 改为 Owner-scoped Disclosure、durable buffer/flush、
Markdown 语义真相源与可重建 SQLite FTS Projection；该段是规划，不是当前运行时事实。详细边界见
...
当前架构状态是 Phase 5 **IMPLEMENTATION PASS**：520 Python tests、30 TypeScript、28/28 Agent、32/32 Channel 与
...
独立 live gate。下一条产品主线是 Phase 5.2 真实稳定化，完成 Feishu 15/15、系统服务和 24 小时 soak 后，再进入
Phase 6 自治运行与 Sandbox；Phase 7 才交付受控自我迭代。任何 production verified 声明仍要先完成对应平台
live exit gate。完整后续 Gap 与模块路线见
[OpenClaw / Hermes 能力 Gap](20260808_OpenClaw-Hermes能力Gap与演进路线.md)和
[能力对齐工程落地总方案](../engineering/openclaw-hermes-alignment-engineering-roadmap.md)。飞书运行时与常驻边界见
...
[Memory、Skills 与上下文压缩工程文档](../engineering/phase-3/memory-skills-compaction.md)；规划中的记忆重构见
