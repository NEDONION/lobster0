...
> 文档日期：2026-08-14
> 状态：**DESIGN**
> 触发事件：飞书图片能力连续**三次**静默失效。第一次是给 SDK 传了不存在的构造关键字
> （Gateway 起不来，日志只有一句 `gateway startup or runtime failed`）；第二次是读了
> `ResourceDescriptor` 上不存在的字段（永远拿到空列表，没有任何报错，只有 Owner 在 IM 里
> 看到"我看不到图片"）；第三次是挂图早于上下文压缩，压缩重建请求时把图片扫掉了
> （见 2.2.1，#58 已修）。**三次都通过了全部单元测试。**
>
> 第三次的代价不止是"看不到图"：Owner 反复追问后，模型开始顺着话编造画面细节，
> 随后又自行承认是编的。静默失效最终变成了可信度问题。
...
### 2.2.1 补记：本文写完之后才查出的真正根因（2026-08-14 16:30）

本文初稿把重点放在"图片有没有被取回"，但当天下午定位到的根因在更后面一步：

**挂图发生在上下文压缩之前，压缩命中时会把 `request` 整个重建，图片随之消失。**

Owner 的会话有 972 条消息，每一轮都触发压缩，于是图片每次都被丢掉。已在 #58 修复
（把挂图移到压缩重建之后）。

这条教训直接影响第 4 节的分层设计，必须写进来：

> **`ModelPart` 的挂载点必须是"请求最终定型的那一刻"，不能是"请求第一次建好的那一刻"。**
> 任何会重建 `request` 的环节——压缩、重试、子 Agent 派发——都会把挂早了的媒体扫掉。

排查耗时长的原因也值得记：链路上每一段都单独验证过且都是对的（下载 ✓、观测 ✓、
穿过 Manager ✓、视觉后端配置 ✓），**唯独丢图的那一步没有任何断言**。这正是第 13 节
要求"跨层集成断言"的由来——分层测试全绿并不能证明合起来是通的。
...
三次事故都**通过了全部单元测试**，但原因分成两类：

- 前两次是**假件比真实 SDK 宽松**：第一次假件接受了真 SDK 不接受的构造关键字，
  第二次假件自带了真 SDK 里不存在的字段。
- 第三次（#58，压缩丢图）假件没有问题，**分层测试也全部为真**——问题在于
  没有任何测试跨越"挂图"与"压缩"这两层。每一层都对，合起来是错的。

第三次尤其值得记：排查时逐段验证了下载、观测、Manager 透传、视觉后端配置，全部通过，
于是很自然地得出"链路是通的"这个错误结论。**分层全绿不等于端到端可用。**

因此本设计要求：

0. **跨层集成断言**：媒体从入口到"请求最终定型"必须有至少一条端到端断言，而不是
   只在每层内部各测各的。具体到本次：`tests/test_turn.py` 断言压缩重建之后请求上
   仍带着图片；`tests/test_channel_manager.py` 断言 `image_paths` 真的穿过管理器
   （此前假 Service 收下这个参数却从不记录，等于从未验证）。
