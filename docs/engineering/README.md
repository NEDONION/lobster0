...
| [Phase 7 工程落地方案](phase-7/20260810_controlled-evolution.md) | Feedback、受限 Prompt/Skill/Memory Proposal、Eval、Owner Approval、Apply/Rollback。 | `IMPLEMENTATION IN PROGRESS`；Owner 明确决定在 Phase 6 生产验收前提前开工。Task 1/6 完成；Task 2 的 CLI、命令解析与飞书 `/good`/`/bad` 已全部接入并在真机验证过三轮（依次修掉：进程未重启、SDK 回复字段读错、卡片平台 ID 未落库）；Task 3 的 Prompt/Skill 校验器完成，Memory candidate 仅支持 forget；Task 4 的确定性 Gate 与 receipt 绑定完成，failure case 已可由 `feedback export-case` 生成为待 Owner 补齐的 planned/live 草稿（离线 Runner 的脚本化回复无法证明行为改进，故不自动入 active 门禁），baseline/candidate 差分仍缺；Task 5 的 Owner Approval（域内 `evolution_approvals` 表，schema v8）、原子 apply/rollback、崩溃恢复与 Runtime 每 Turn 读取已完成，Prompt 与 Skill 两类目标均已端到端可用（Skill 候选按内容哈希落进 `skills/versions` 不可变版本库，Runtime 可读取生效版本并在 artifact 损坏时 fail safe），Memory 目标的 apply 委托给既有 MemoryReviewService 落地（穿过 disclosure/conflict/forget/reconcile 规则，不自己改 Markdown truth），并按设计显式拒绝 rollback——改动已落进 Markdown truth，切指针撤不回来，谎称成功比拒绝更危险；Task 6 的 evolve CLI 与 15 条 versioned evolution case（20 轮 soak 300/300）已完成，飞书 Proposal 摘要卡已实现（只展示目标/状态/评测结论/候选指纹，不含完整 diff、不含任何执行按钮——apply 只能走本机 Core Approval），但尚未接入 Delivery；real Provider 与 Owner 飞书 controlled smoke 仍为 pending。 |
