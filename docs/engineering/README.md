...
| [Phase 2.2B：单入口 Textual TUI](phase-2/single-entry-tui.md) | Textual 选型、唯一入口、AgentRuntime、RunEvent、Worker、Tool 卡与 Allow once/Deny Modal。 | HTTP、飞书、永久审批规则。 |
...
功能主线当前位于 P2.3B：P2.2B 单入口 TUI 与 P2.3A exact-argv `run_command` 已验证（全仓
232 tests）；下一步处理 NVM/Node 运行时路径、doctor 检查，并用本机 `lark-cli auth status` 跑通第一条
真实受控命令。质量主线下一步仍是 R3 live DeepSeek release eval。
