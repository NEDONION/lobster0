...
> [!WARNING]
> 文档性质：`HISTORICAL SNAPSHOT`。Phase 2.2B 已移除 `lobster0 chat`、one-shot 与 `input()` REPL。
> 当前人类对话入口是裸 `lobster0` 的默认 pi-tui，Textual 仅作 fallback。请优先阅读
> [单入口 TUI 工程文档](../phase-2/20260808_single-entry-tui.md)。

## 1. 模块目的

`src/lobster0/cli.py` 是 Phase 1 当时的用户入口。该阶段在已有 `init`、`doctor` 之上增加：

```bash
lobster0 chat --message TEXT [--session ID] [--home /absolute/path]
lobster0 chat [--session ID] [--home /absolute/path]
```
...
| 参数 | 类型 | 默认值 | 行为 |
| --- | --- | --- | --- |
| `--home` | 绝对路径 | `LOBSTER0_HOME` 或 `~/.lobster0` | 由 `resolve_home()` 校验 |
...
```mermaid
flowchart TD
    START["lobster0 chat"] --> ARGS["argparse 校验参数"]
...
`chat` 只接受已经初始化的状态根。缺少 `config.toml`、`lobster0.db`、`SOUL.md` 或 `USER.md`，以及这些
文件是符号链接时，CLI 返回：

```text
error: Lobster0 is not initialized at ...; run lobster0 init first
```
...
```bash
uv run lobster0 chat --message "只回答：Lobster0 online"
uv run lobster0 chat --session research --message "记住我们正在研究 Agent 架构"
```
...
```bash
answer=$(uv run lobster0 chat --message "hello")
status=$?
```
