...
> `tests/test_agent_runner.py` 共同构成版本门禁。当前全仓基线：253/253 tests、20/20 offline
...
- Provider 返回非空 `reasoning_content` 时，生成 `Reasoning (provider)` 卡；
...
| `TUI-011` | 危险动作审批 | 完整归一化参数、Deny 默认焦点、只 Allow once | `test_tui` |
