...
## 4.11 承诺对照核查（2026-08-12）

D1～D3 全部标记完成后，逐条把设计文档的承诺与真实代码对照了一遍。**八处「说了
但没接上」**，其中六处是在这次核查里发现的，此前全靠碰巧撞见：

| # | 承诺 | 实际情况 |
| --- | --- | --- |
| 1 | 附件抵达模型（D2c §3.5） | `attachment_ids` 校验完就丢弃 |
| 2 | 「查看完整过程」可点 | 按钮做了，`app.tsx` 没传回调，渲染条件永不成立 |
| 3 | 失败运行留下证据 | 只有成功分支写 `result_preview`/`usage` |
| 4 | 右栏显示当前任务产物（D3 §6.1） | `link()` 只有用户上传一处调用，Agent 产物从不关联 |
| 5 | 「在访达中显示」（D3 §4） | 路径存进一个没人读的字段，Main 从没调 `showItemInFolder` |
| 6 | 模型名/Workspace 可点（D2c §4.2） | 状态栏一直是纯 `span` |
| 7 | 一键重启 Core（D2b §6） | 只有文字提示，没有按钮 |
| 8 | 侧栏及时反映新对话（D1） | effect 只依赖 bootstrap，整个生命周期只拉一次 |

### 共同特征

八处**没有一处是逻辑写错**——每个部件单独看都对，测试也都是绿的。问题全部出在
**部件之间的接缝**：组件没被传参、字段没人读、写操作只覆盖了一半分支。

单元测试天然测不到这类问题：它验证的是"组件在被正确调用时行为正确"，而不是
"组件真的被调用了"。
...
1. 这个能力对应的 API 在 renderer 里被调用了吗？（`grep` 函数名，出现 0 次即缺口）
2. 调用产生的数据有人消费吗？（`_pending_reveal` 就是这样露馅的）
3. 写路径的**所有分支**都覆盖了吗？（失败分支漏写 usage 就是这样）

三条都是纯静态检查，成本极低，但这次一口气找出六个缺口。

**核查本身也会出错。** 我一度判定 `aria-current="page"` 缺失并准备动手，实际上它
是用 JSX 表达式写的（`aria-current={... ? "page" : undefined}`），只是躲过了按
字面量的 grep。教训：对"缺失"的判断要按**属性名**而不是按完整字面量搜索，并且在
动手改之前先把那段代码读一遍。D1 的其余承诺（无障碍属性、连接中的 placeholder、
空态文案）经复核全部落实。
