...
当前版本已完成默认 pi-tui、Memory Autopilot 与 Phase 6 Autonomy/Sandbox 的代码/离线门禁，状态为
...
Checkpoint 只覆盖主 Workspace，Rollback 暂无 CLI/TUI；Browser Agent 尚未实现。
...
预期 Agent 39/39、Channel 33/33、Automation 15/15。Eval 不读取 `.env` 或个人状态；Personal case 使用临时 Home、临时多根和
...
当前门禁规模：**IMPLEMENTATION PASS**；798/798 Python tests、35/35 TypeScript tests、39/39 active offline Agent
cases、33/33 Channel cases、660/660 local Channel soak、15/15 Automation 与 300/300 Automation soak。Feishu 是
