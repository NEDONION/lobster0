...
当前版本已完成默认 pi-tui 与 Phase 5.1 的代码/离线门禁，状态为 **IMPLEMENTATION PASS**。Feishu 是
...
Personal Profile、NVM/Node 路径发现和 `lark-cli --version` 只读纵切已经支持；Telegram/Discord 的代码与离线
门禁也已完成。尚未支持：Cron/Heartbeat、OS Sandbox、Browser Agent、跨会话检索、自动演进、MCP、Sub-agent
与多模态。后续顺序和完成定义见
[OpenClaw / Hermes 能力 Gap](../architecture/20260808_OpenClaw-Hermes能力Gap与演进路线.md)及
[能力对齐工程落地总方案](../engineering/openclaw-hermes-alignment-engineering-roadmap.md)。飞书专用
App/Bot、认证、Scope、Owner allowlist、WebSocket ready
...
预期 Agent 28/28、Channel 32/32。Eval 不读取 `.env` 或个人状态；Personal case 使用临时 Home、临时多根和
...
当前门禁规模：**IMPLEMENTATION PASS**；520/520 Python tests、30/30 TypeScript tests、28/28 active offline Agent
