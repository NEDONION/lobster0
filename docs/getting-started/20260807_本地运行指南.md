...
- 24 条 Claw-like query 的离线 Agent 回归。
...
尚未支持：NVM/Node 路径下 `lark-cli` 的真实 smoke、向量检索与自动演进。本机尚未完成三个 IM 的真实平台
验收；Telegram 和 Discord 均为 **LIVE PENDING**。
...
目录、TOML、Workspace、SQLite integrity/schema v2、POSIX 权限、Node/pi-tui、三个 IM 配置、SDK 和凭据变量，
不会修复或联网。当前固定 20 项检查。
...
```dotenv
MINICLAW_TELEGRAM_BOT_TOKEN=replace-with-real-token
```
...
```dotenv
MINICLAW_DISCORD_BOT_TOKEN=replace-with-real-token
```
...
预期 Agent 24/24、Channel 32/32。Eval 不读取 `.env` 或个人状态；Agent case 使用独立临时 state 与
ScriptedProvider，Channel case 调用真实 Adapter、Repository、Worker、Approval、Delivery 和错误映射。
...
当前基线：**IMPLEMENTATION PASS**；456/456 Python tests（1 项预期 skip）、25/25 TypeScript tests、24/24 active
...
[Phase 2.3A exact-argv 工程文档](../engineering/phase-2/command-execution.md)；P2.3B 的
`lark-cli`/Node 路径与真实 `auth status` smoke 仍是独立待办。Phase 4 Channel 细节见
