...
`reasoning_content`，同样会以 `Reasoning (provider)` 折叠卡保留；没有该字段时不伪造思考过程。
...
默认 `allowlist + on-miss` 会先校验 URL 与全部 DNS 地址，再显示一次性审批。只允许 HTTPS GET，不提供
...
当前基线：**258/258 tests、21/21 active offline Agent cases、Ruff PASS**。
