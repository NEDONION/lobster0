...
- 20 条 Claw-like query 的离线 Agent 回归。
...
预期 20/20 PASS。Offline eval 不读取 `.env` 或个人状态；每条 case 使用独立临时 state 与 ScriptedProvider，
...
当前基线：**253/253 tests、20/20 active offline Agent cases、Ruff PASS**。
