...
`reasoning_content`，会以弱色、无厚边框、默认折叠的 reasoning 行保留；没有该字段时不伪造思考过程。
模型被要求让可见 reasoning 跟随当前用户提问语言，UI 语言不会二次翻译模型内容。
...
当前基线：**296/296 tests、24/24 active offline Agent cases、Ruff PASS**。
