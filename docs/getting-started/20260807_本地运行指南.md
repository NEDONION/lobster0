...
当前版本已完成 Phase 3 与默认 pi-tui；Phase 4 飞书 Channel Core 与 Gateway 已进入代码：

```text
pi-tui → versioned NDJSON Bridge → AgentRuntime → TurnService → AgentRunner → DeepSeek / ToolExecutor → SQLite
```
...
- 飞书 normalized message、allowlist、durable Inbox/Outbox、官方 WebSocket Transport、分片重试、卡片审批与 `miniclaw gateway`；
- 24 条 Claw-like query 的离线 Agent 回归。

尚未支持：真实飞书账号 E2E/部署/soak、NVM/Node 路径下 `lark-cli`
真实 smoke、Telegram/Discord Channel、向量检索与自动演进。飞书 Core 的实现与边界见
[Phase 4 工程文档](../engineering/phase-4/feishu-channel-core.md)。
...
Python 运行依赖包含 HTTPX 与迁移期 Textual fallback；TypeScript TUI 固定 pi-tui 0.84.1。唯一脚本入口仍由 `pyproject.toml` 提供：
...
目录、TOML、Workspace、SQLite integrity/Schema、POSIX 权限、Node 版本和 pi-tui 构建入口，不会修复或联网。
...
预期 Offline 24/24、Channel 12/12 PASS。两套 eval 都不读取 `.env` 或个人状态；每条 case 使用独立临时 state 与 ScriptedProvider/固定 fixture，
...
当前基线：**382/382 Python tests、25/25 TypeScript tests、24/24 active offline Agent cases、12/12 Channel cases、Ruff PASS**。
