...
当前版本已完成 Phase 3 与默认 pi-tui；Phase 4 飞书 Channel Core 已进入代码：
...
- 飞书 normalized message、allowlist、durable Inbox/Outbox、官方 WebSocket Transport、分片重试和卡片审批 Core；
- 24 条 Claw-like query 的离线 Agent 回归。

尚未支持：正式 `miniclaw gateway` 启动器、真实飞书账号 E2E/部署/soak、NVM/Node 路径下 `lark-cli`
...
预期 24/24 PASS。Offline eval 不读取 `.env` 或个人状态；每条 case 使用独立临时 state 与 ScriptedProvider，
...
当前基线：**369/369 Python tests、25/25 TypeScript tests、24/24 active offline Agent cases、Ruff PASS**。
