...
尚未支持：NVM/Node 路径下 `lark-cli` 的真实 smoke、Telegram/Discord、向量检索与自动演进。本机尚未配置
飞书企业应用凭据，所以真实平台验收仍待执行。
...
目录、TOML、Workspace、SQLite integrity/schema v2、POSIX 权限、Node/pi-tui、飞书配置、SDK 和凭据变量，不会修复或联网。
...
预期 Agent 24/24、Channel 12/12。Eval 不读取 `.env` 或个人状态；Agent case 使用独立临时 state 与
...
当前基线：**391/391 Python tests、25/25 TypeScript tests、24/24 active offline Agent cases、12/12 Feishu Channel cases、Ruff PASS**。
