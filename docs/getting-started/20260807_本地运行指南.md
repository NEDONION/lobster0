...
- 21 条 Claw-like query 的离线 Agent 回归。

尚未支持：NVM/Node 路径下 `lark-cli` 的真实 smoke、飞书/Telegram/Discord Channel、长期
...
预期 21/21 PASS。Offline eval 不读取 `.env` 或个人状态；每条 case 使用独立临时 state 与 ScriptedProvider，
...
当前基线：**295/295 Python tests、25/25 TypeScript tests、21/21 active offline Agent cases、Ruff PASS**。
