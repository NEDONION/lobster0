...
- 产品工作名：MiniClaw
- 版本：PRD v0.1
- 状态：开发中（Phase 2 Tool、安全、TUI 可观测、分级审批、恢复和回归门禁已完成；飞书尚未开始）
- 日期：2026-08-07
...
1. 用户运行裸 `miniclaw`，进入唯一的 Textual 全屏 TUI。
2. TUI 通过同进程 `AgentRuntime` 把输入交给 `TurnService`。
...
外层命令使用 Python 标准库 `argparse` 和 `pyproject.toml` 脚本入口；本地交互使用 Textual。产品不提供
...
默认中文/英文切换、真实 usage 审计栏，以及 Core 受限的 Once/Session/Always 审批。飞书与 `gateway` 仍是
后续里程碑，不能从本地 TUI 完成推断为已交付。
...
| 语言 | Python 3.12 |
| 包管理 | `uv` |
| CLI / TUI | `argparse` + Textual |
| 飞书 | `lark-oapi` |
| 数据库 | SQLite / `sqlite3` |
| 模型 | 一个 OpenAI-compatible HTTP Provider |
| 配置 | TOML + 环境变量 |
| 异步 | `asyncio` + Textual Worker；飞书阶段增加 `asyncio.Queue` |
| 测试 | 标准库 `unittest` |
| 部署 | Docker Compose |
| License | MIT |
...
当前阶段证据：295/295 Python、25/25 TypeScript 确定性测试与 21/21 active offline Agent cases 通过；这证明本地 Agent Core/TUI
回归，不代表飞书、Memory/Skills、自我演进和 Docker 验收已完成。
