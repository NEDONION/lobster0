...
- 产品工作名：MiniClaw
- 版本：PRD v0.1
- 状态：开发中（Phase 5.1 **IMPLEMENTATION PASS**；Feishu **FEISHU OWNER-DM DELIVERY VERIFIED / 15-CASE LIVE PENDING**；
...
- `write_file` / `edit_file`：只写 Workspace 或 Personal 的有限写根，每次都需要参数绑定审批。
...
当前阶段证据：Phase 5.1 **IMPLEMENTATION PASS**；520/520 Python tests、30/30 TypeScript tests、28/28 active
...
[能力对齐工程落地总方案](../engineering/openclaw-hermes-alignment-engineering-roadmap.md)为权威路线。下列 Phase
均为规划，只有合并代码、确定性门禁和必要 live evidence 同时存在时才更新为完成。
...
- 完成 Feishu 15/15、系统服务管理、24 小时 soak 和脱敏 Evidence；
- 完成非 root Docker Compose 与服务 health；
- 只在真实证据存在后把状态从 `LIVE PENDING` 更新为 `LIVE VERIFIED`。
