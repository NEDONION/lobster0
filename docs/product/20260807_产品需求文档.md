...
- 产品工作名：MiniClaw
- 版本：PRD v0.1
- 状态：开发中（Phase 5.2 **IMPLEMENTATION PASS**；Feishu **FEISHU OWNER-DM DELIVERY VERIFIED / POST-FIX
  SINGLE-CARD + 15-CASE LIVE PENDING**；
  Telegram/Discord **LIVE PENDING**；Evolution 与部署尚未开始）
- 日期：2026-08-07
...
- TUI、飞书、Telegram 和 Discord 复用同一个个人 Agent Core，但当前仍保留各自会话历史。
- 当前 Memory v1 只注入 `MEMORY.md`、今日/昨日 daily memory 和当前 Session 的持久化摘要；跨渠道 Owner
  Memory Space、跨 Session 检索、自动 Flush/晋升/冲突/遗忘属于 Memory Autopilot A～E，尚未实现。
- 群聊 Channel 已实现，但默认不得召回 Owner 私人记忆；非 Owner 和身份不确定入口同样 fail closed。
- 记忆写入必须可追溯日期和来源会话；API Key、Token、验证码、密码和私钥永不保存。
...
当前阶段证据：Phase 5.2 **IMPLEMENTATION PASS**；562/562 Python tests、30/30 TypeScript tests、29/29 active
offline Agent cases、32/32 Channel cases 与 20 轮 640/640 local soak。Feishu 专用 App/Bot、Scope、Owner allowlist、
WebSocket ready 与两条 Owner 私聊 Delivery 已经真实验证，状态是
**FEISHU OWNER-DM DELIVERY VERIFIED / 15-CASE LIVE PENDING**；Telegram 与 Discord 真实平台均为 **LIVE PENDING**。
这证明本地 Agent Core/TUI、Memory/Skills/compaction、fake transport 全链路，以及飞书 Owner DM 的真实
Adapter、Inbox、Agent、Outbox 和回复闭环；它不代表完整 15-case、Evolution、Docker、24×7 soak 或当前版本
...
[能力对齐工程落地总方案](../engineering/openclaw-hermes-alignment-engineering-roadmap.md)为权威路线。下列 `M` 是
产品交付里程碑，不等同于架构 Phase；真实交付顺序见[开发与交付时间线](../engineering/development-timeline.md)。
...
- 前置：先完成 Memory Autopilot A～E 的跨渠道 Identity、Disclosure、Flush、Recall 与治理基础；
- one-shot、interval、cron、Heartbeat 和 durable Task Ledger；
- 主动 Channel 投递、重启恢复、幂等和任务预算；
- Docker/Seatbelt Sandbox 与工作区 Checkpoint/rollback。

### M6 / Phase 6.5：Browser Agent

- MiniClaw 专用 Chromium Profile；
- snapshot/ref、点击、输入、截图、下载和浏览器审批；
- 网页 Prompt Injection、不可信内容与 Artifact 回归。
