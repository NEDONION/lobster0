...
- 状态：开发中（Phase 6 Autonomy/Sandbox 与 Phase 6.5 Browser **IMPLEMENTATION PASS**；Docker **LIVE VERIFIED**；
  ...
  macOS + 飞书 production tooling **IMPLEMENTATION PASS / PRODUCTION SOAK PENDING**；
...
Lobster0 是一个面向单个用户的自托管个人 Agent。用户可以在本地 TUI、Desktop、飞书、Telegram 或 Discord 中与同一个
...
Lobster0 不追求复制 OpenClaw 的完整功能。v0.1 的目标是用一套规模可控、容易阅读的 Python 代码，跑通个人 Agent 的完整工程闭环：消息接入、Agent Loop、工具执行、记忆、权限、反馈、评测、迭代和部署。
...
1. 用户安装 Python 包并运行 `lobster0 init`。
...
5. `lobster0 doctor` 检查模型、数据库、Workspace 和飞书连接配置。

### 6.2 本地 TUI 对话

1. 用户运行裸 `lobster0`，默认进入唯一的 TypeScript pi-tui；依赖不可用时进入 Textual fallback。
...
2. Lobster0 保存原始输入、上下文摘要、工具轨迹、输出和反馈。
3. `lobster0 evolve propose` 根据失败案例生成 Prompt 或 Skill 修改提案。
4. `lobster0 eval --proposal <id>` 对基准案例和失败案例做回放。
5. CLI 展示修改前后通过率、耗时、Token 使用量和安全违规。
6. 用户执行 `lobster0 evolve apply <id>` 后，新版本生效。
7. 用户可以执行 `lobster0 evolve rollback` 恢复上一版本。
...
| 命令 | 作用 |
| --- | --- |
| `lobster0` | 唯一的本地对话入口；默认 pi-tui，Textual 仅作 fallback |
| `lobster0 init` | 创建配置、Workspace 和数据库 |
| `lobster0 gateway` | 启动飞书 Gateway 和消息 Worker |
| `lobster0 doctor` | 检查配置、模型、权限和连接 |
| `lobster0 sessions` | 查看本地会话列表 |
| `lobster0 eval` | 运行回归评测 |
| `lobster0 evolve propose` | 生成改进提案 |
| `lobster0 evolve apply <id>` | 应用已通过评测的提案 |
| `lobster0 evolve rollback` | 回滚最近一次改进 |

外层命令使用 Python 标准库 `argparse` 和 `pyproject.toml` 脚本入口；默认本地交互使用 TypeScript pi-tui，
Python Textual 暂作 onboarding/fallback。产品不提供
`lobster0 chat`、`lobster0 tui`、one-shot 或另一套 plain REPL。

当前本地 TUI 已实现：用户/Agent 文字角色标签、流式 Markdown、紧凑 reasoning、Tool Trace、长文本失败恢复、
默认中文/英文切换、真实 usage 审计栏，以及 Core 受限的 Once/Session/Always 审批。`lobster0 gateway`、
...
flowchart LR
    ...
    subgraph RUNTIME["Lobster0 Runtime"]
        QUEUE["Message Queue"]
...
- 在 macOS 或 Linux 上运行 `lobster0 gateway`。
- 飞书使用出站 WebSocket，不要求公网 IP、域名或入站端口。
- SQLite、Workspace 和日志保存在用户指定的数据目录。

### 11.2 常驻运行

- macOS 个人常开机使用用户级 `launchd` 启动 `lobster0 gateway`；Secret 不写入 plist。
...
- 配置或数据库不可用时 Gateway 拒绝启动，并由 `lobster0 doctor` 给出修复建议。
...
当前阶段证据：Phase 6.5 **IMPLEMENTATION PASS**；1005/1005 Python tests、41/41 TUI TypeScript、
...
Feishu 15-case、同 commit Seatbelt 2-case、Browser controlled public live smoke、Evolution、24×7 soak 或当前版本
...
- 裸 `lobster0` 单入口，以及 `init`、`doctor` 维护命令。
- 状态：已实现。
...
- CLI 端到端对话与最小测试。
- 状态：已实现；旧 `lobster0 chat` 已由裸 `lobster0` pi-tui 入口替代。
...
- production invariant monitor、exact 24h checkpoint、同 commit Evidence 聚合和受管 recovery orchestrator 已实现；
- 当前状态：**IMPLEMENTATION PASS / PRODUCTION SOAK PENDING**。严格飞书 15/15、Automation 10/10、同 commit
  Seatbelt 2/2 和连续 24 小时尚未共同通过；Heartbeat 尚无 Owner IM route，Checkpoint 只覆盖主 Workspace，
  Rollback 暂无 CLI/TUI。执行手册见[macOS + 飞书生产级验收](../engineering/phase-6/20260810_macos-feishu-production-acceptance.md)。

### M6 / Phase 6.5：Browser Agent

- 已实现 Lobster0 专用 Chromium Profile、排他锁、tab/idle 上限和进程组清理；
...
### Desktop Workbench 横向交付轨道（W0/W1 基线，D1～D5 规划）

- W0/W1 development build 保留为已实现基线：同仓 Electron + React，复用 Python Bridge、Policy、Storage
  和 Automation；
- 新目标以 LobsterAI Cowork 为主体，打开应用后首屏直接显示真实 Composer；
- Composer 将接入附件、模型、Workspace 和 Agent 选择，右栏展示真实 Artifact；
- OpenAgents 的 Agent 列表、任务线程、参与状态和共享产物只作为信息架构参考；
- D4 消费 Phase 9 depth-1 Sub-agent，不引入第二套 Runtime、Policy 或 SQLite；
- 当前状态：`TARGET CONFIRMED / D1-D5 IMPLEMENTATION PENDING`。完整需求、架构和顺序见
  [桌面多 Agent 工作台开发需求](20260810_桌面多Agent工作台开发需求.md)、
  [LobsterAI-first 桌面多 Agent 设计](../architecture/20260810_LobsterAI-first桌面多Agent设计.md)和
  [D1～D5 分 Phase 落地方案](../engineering/desktop/20260810_桌面多Agent分Phase落地.md)。
...
- 作者连续 30 天使用 Lobster0，并每周至少产生 10 次有效对话。
...
项目正式工作名为 **Lobster0**，仓库名、Python 导入包和 CLI 统一使用 `lobster0`。
