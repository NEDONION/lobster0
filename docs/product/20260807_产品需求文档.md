...
- 产品工作名：MiniClaw
- 版本：PRD v0.1
- 状态：开发中（Phase 2/3 与默认 pi-tui 已完成；Phase 4 飞书 Channel Core 与 Gateway 已实现，真实 E2E 待完成）
- 日期：2026-08-07
...
- Agent Core 使用 Python 3.12；本地默认 TUI 允许 Node/TypeScript 异构进程，IM Gateway 仍保持单 Python Runtime。
- 提供可安装的 `miniclaw` CLI。
- 支持本地终端连续对话。
...
默认中文/英文切换、真实 usage 审计栏，以及 Core 受限的 Once/Session/Always 审批。飞书 Channel Core、
durable Inbox/Outbox、官方 WebSocket Transport、卡片审批和 `gateway` CLI 已实现；真实账号 E2E 与部署仍是
后续里程碑，不能从离线回归推断为生产可用。
...
| 包管理 | `uv` |
| CLI / TUI | `argparse` launcher + `@earendil-works/pi-tui`；Textual fallback |
| 飞书 | `lark-oapi` |
| 数据库 | SQLite / `sqlite3` |
| 模型 | 一个 OpenAI-compatible HTTP Provider |
| 配置 | TOML + 环境变量 |
| 异步 | Python `asyncio` + NDJSON Bridge；pi-tui 事件循环；飞书阶段增加 `asyncio.Queue` |
...
当前阶段证据：382/382 Python、25/25 TypeScript 确定性测试与 24/24 active offline Agent cases、12/12 Channel cases 通过；这证明本地 Agent Core/TUI、
Markdown Memory、Skills 惰性激活和 persistent compaction 回归，不代表飞书、自我演进、Docker 或当前版本
真实 DeepSeek release smoke 已完成。
