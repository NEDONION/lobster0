...
class FeishuLiveHarnessSafetyTest(unittest.TestCase):
    ...
    def test_live_environment_honors_installed_secret_file(self) -> None:
        """隔离 worktree 必须复用 Gateway 的绝对 MINICLAW_ENV_FILE。"""
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            secret_file.write_text(
                f"MINICLAW_HOME={home}\nMINICLAW_FEISHU_APP_ID=fake-app\n",
                encoding="utf-8",
            )
            secret_file.chmod(0o600)
            with patch.dict(
                os.environ,
                {"MINICLAW_ENV_FILE": str(secret_file)},
                clear=True,
            ):
        ...
        self.assertEqual(environment["MINICLAW_FEISHU_APP_ID"], "fake-app")
