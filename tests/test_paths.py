...
from lobster0.paths import PathConfigurationError, build_state_paths, resolve_home
...
class StatePathsTest(unittest.TestCase):
    ...
    def test_explicit_home_overrides_environment(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as explicit_directory:
            resolved = resolve_home(
                explicit_directory,
                {"LOBSTER0_HOME": "/ignored/by/explicit/value"},
            )
    ...
    def test_default_home_uses_dot_lobster0(self) -> None:
        """没有覆盖值时应使用当前用户主目录下的固定状态目录。"""
        resolved = resolve_home(None, {})

        self.assertEqual(resolved, (Path.home() / ".lobster0").resolve())
