...
这个仓库经常有多个 Agent 会话同时在改。已经发生过两次真实事故，因此下面几条是硬要求：

- **禁止 `git add -A`、`git add .`、`git commit -a`。** 只能 `git add <明确列出的文件>`。
  一次事故里，某个会话用全量暂存把另一个会话正在写的 Phase 7 代码卷进了标题完全无关的
  `fix(bridge): ...` 提交，导致这批代码散落在四个不相关的提交里，`git log` 无法追溯。
- **暂存前先看清楚**：`git status --short` 里出现你这次没有编辑过的文件，就是别的会话在工作，
  不要暂存它，也不要 stash 它。
- **不要 stash 别人的改动**。另一次事故里，会话为了"保护工作区"stash 了另一个会话正在编辑的
  `config.py`，导致对方的改动凭空消失。需要干净工作区时，只提交自己那几个文件即可。
- **提交前后各跑一次 `git fetch` + `git status`**，确认自己没有落后于别人刚推的提交。
- 长任务优先用 `git worktree` 隔离，从源头避免共享工作区。
